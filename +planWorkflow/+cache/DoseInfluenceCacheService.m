classdef DoseInfluenceCacheService
    ...
    methods (Access = private)
        ...
        function dij = loadCompactRef(obj,ref,owner,~,cacheKind)
            variables = ...
                planWorkflow.cache.DoseInfluenceCacheService.compactVariables( ...
                cacheKind);
            persistedFile = obj.refCacheFile(ref);
            cacheFile = persistedFile;
            variableName = char(ref.variableName);
            planWorkflow.cache.DoseInfluenceCacheService.assertCompactRefVariable( ...
                ref,variableName,variables,cacheFile);

            cachedMetadata = ...
                planWorkflow.cache.DoseInfluenceCacheService.loadCacheFile( ...
                cacheFile,{'cacheMetadata'},ref.role);
            planWorkflow.cache.DoseInfluenceCacheService.assertCacheRefMetadata( ...
                ref,cachedMetadata.cacheMetadata,cacheFile);
            rehydrateContext = ...
                planWorkflow.cache.DoseInfluenceCacheService.compactRehydrationContextForRef( ...
                ref,owner);
            switch char(cacheKind)
                case 'interval'
                    [compatible,reason] = ...
                        planWorkflow.precompute.IntervalDoseInfluence.isCacheMetadataCompatible( ...
                        cachedMetadata.cacheMetadata,owner,'rehydrate', ...
                        ref,rehydrateContext);
                case 'prob'
                    [compatible,reason] = ...
            ...
            if ~compatible
                ...
                error(['planWorkflow:cache:DoseInfluenceCacheService:' ...
                    ...
                    ['Cannot rehydrate %s: compact cache is not ' ...
                        'compatible with the persisted dose-influence ref ' ...
                        '(%s). Cache: "%s".'],char(ref.role), ...
                    char(reason),cacheFile);
            end
            cached = ...
                planWorkflow.cache.DoseInfluenceCacheService.loadCacheFile( ...
                cacheFile,{variableName},ref.role);
            cached.cacheMetadata = cachedMetadata.cacheMetadata;
            switch char(cacheKind)
                case 'interval'
                    [compatible,reason] = ...
            ...
            if ~compatible
                ...
                error(['planWorkflow:cache:DoseInfluenceCacheService:' ...
                    'IncompatibleCompactCacheOnResume'], ...
                    ['Cannot rehydrate %s: compact cache variable is not ' ...
                    'compatible with the persisted dose-influence ref ' ...
                    '(%s). Cache: "%s".'],char(ref.role), ...
                    char(reason),cacheFile);
            end
    ...
    methods (Static)
        ...
        function cached = loadCacheFile(cacheFile,variables,role)
            ...
            fileVariables = who('-file',cacheFile);
            for variableIx = 1:numel(variables)
                variableName = char(variables{variableIx});
                if ~any(strcmp(variableName,fileVariables))
                    error(['planWorkflow:cache:DoseInfluenceCacheService:' ...
        ...
        function variables = compactVariables(cacheKind)
            switch char(cacheKind)
                case 'interval'
        ...
        function assertCompactRefVariable(ref,variableName,allowedVariables, ...
                cacheFile)
            if ~any(strcmp(char(variableName),allowedVariables))
                error(['planWorkflow:cache:DoseInfluenceCacheService:' ...
                    'UnsupportedCompactCacheVariable'], ...
                    ['Cannot rehydrate %s from cache "%s": variable "%s" ' ...
                    'is not valid for %s compact cache refs.'], ...
                    char(ref.role),cacheFile,char(variableName), ...
                    char(ref.cacheKind));
            end
            if isfield(ref,'variables') && ~isempty(ref.variables)
                declaredVariables = ...
                    planWorkflow.cache.DoseInfluenceCacheService.normalizeVariables( ...
                    ref.variables);
                if ~any(strcmp(char(variableName),declaredVariables))
                    error(['planWorkflow:cache:DoseInfluenceCacheService:' ...
                        'DijCacheRefVariableMismatch'], ...
                        ['Cannot rehydrate %s from cache "%s": persisted ' ...
                        'ref does not declare variable "%s".'], ...
                        char(ref.role),cacheFile,char(variableName));
                end
            end
        end
        ...
        function context = compactRehydrationContextForRef(ref,owner)
            source = owner;
            if isstruct(ref) && isfield(ref,'role') && ...
                    strcmp(char(ref.role),'optimizationInput') && ...
                    isstruct(owner) && isfield(owner,'optimizationInput') && ...
                    isstruct(owner.optimizationInput)
                source = owner.optimizationInput;
            end
            context = struct();
            fields = {'cst','stf','pln','dijKind','source'};
            for fieldIx = 1:numel(fields)
                fieldName = fields{fieldIx};
                if isstruct(source) && isfield(source,fieldName)
                    context.(fieldName) = source.(fieldName);
                end
            end
        end
        ...
        function variables = normalizeVariables(variables)
