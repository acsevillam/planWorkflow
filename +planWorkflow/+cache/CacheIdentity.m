classdef CacheIdentity
    ...
    methods (Static)
        ...
        function identity = cstGeometryIdentity(cst)
            identity = struct('structures',[]);
            if ~iscell(cst) || isempty(cst)
                return;
            end
        ...
        function clinicalContext = clinicalContext(context)
            clinicalContext = struct('schemaVersion',1);
            if ~isstruct(context)
                return;
            end
            if isfield(context,'cst')
                cstGeometry = ...
                    ...
                    context.cst);
                clinicalContext.cstGeometry = cstGeometry;
                clinicalContext.cstGeometryHash = ...
                    planWorkflow.cache.CacheIdentity.valueHash( ...
                    cstGeometry);
                clinicalContext.cstHash = ...
                    planWorkflow.cache.CacheIdentity.valueHash( ...
                    planWorkflow.cache.CacheIdentity.cstIdentity( ...
                    context.cst));
            end
            if isfield(context,'stf')
                clinicalContext.stfHash = ...
                    planWorkflow.cache.CacheIdentity.valueHash( ...
                    context.stf);
            end
        end
