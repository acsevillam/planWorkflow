...
function testProstatePhotonMultipleWcScenWrapperSelectsWithWcScen( ...
        testCase)
macroRoot = testCase.TestData.macroRoot;
wrapperFile = fullfile(macroRoot,'FSvsDS_01','prostate','wcScen', ...
    'runProstatePhotonMultipleWorkflow_wcScen.m');
wrapperText = fileread(wrapperFile);

verifyNotEmpty(testCase,regexp(wrapperText, ...
    'optimizationScenario\s*=\s*struct\(\s*\.\.\.\s*''mode'',\s*''wcScen''', ...
    'once'));
verifyNotEmpty(testCase,regexp(wrapperText, ...
    '''robustScenario'',\s*optimizationScenario', ...
    'once'));
verifyEmpty(testCase,regexp(wrapperText, ...
    'ScenarioSpec\.normalize\(\s*\.\.\.\s*robustOptimizationScenario', ...
    'once'));
end
