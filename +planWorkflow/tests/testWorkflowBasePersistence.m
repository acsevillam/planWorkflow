...
for modeIx = 1:numel(modes)
    ...
    if isfield(expectedRef,'scenarioFingerprint')
        verifyEqual(testCase, ...
    ...
    if isfield(expectedRef,'numOfScenarios')
        verifyEqual(testCase, ...
...
function testCompactDijRefsAllowLegacyRefWithoutScenarioFingerprint( ...
        testCase)
modes = {'INTERVAL2','INTERVAL3','PROB2'};
for modeIx = 1:numel(modes)
    mode = modes{modeIx};
    ...
    compactData.optimizationInput.pln.multScen = ...
        driftedWorkflowScenarioModel();
    if isfield(compactData.optimizationInput.dijRef,'scenarioFingerprint')
        compactData.optimizationInput.dijRef = rmfield( ...
            compactData.optimizationInput.dijRef,'scenarioFingerprint');
    end
    if isfield(compactData.optimizationInput.dijRef,'numOfScenarios')
        compactData.optimizationInput.dijRef = rmfield( ...
            compactData.optimizationInput.dijRef,'numOfScenarios');
    end
