...
for modeIx = 1:numel(modes)
    ...
    verifyTrue(testCase, ...
        isfield(compactData.optimizationInput.dijRef,'cstHash'),mode);
    verifyTrue(testCase, ...
        isfield(compactData.optimizationInput.dijRef,'stfHash'),mode);
    verifyTrue(testCase, ...
        isfield(compactData.optimizationInput.dijRef, ...
        'cstGeometryHash'),mode);
    ...
    if any(strcmp(char(mode),{'INTERVAL2','INTERVAL3','PROB2'}))
        verifyTrue(testCase, ...
            isfield(compactData.optimizationInput.dijRef, ...
            'payloadContextHash'),mode);
        verifyTrue(testCase, ...
            isfield(compactData.optimizationInput.dijRef, ...
            'scenarioFingerprint'),mode);
        verifyTrue(testCase, ...
            isfield(compactData.optimizationInput.dijRef, ...
            'numOfScenarios'),mode);
        verifyEqual(testCase, ...
            compactData.optimizationInput.dijRef.scenarioFingerprint, ...
            expectedRef.scenarioFingerprint,mode);
        verifyEqual(testCase, ...
            compactData.optimizationInput.dijRef.scenarioFingerprint, ...
            cachedRef.cacheMetadata.scenarioFingerprint,mode);
        verifyEqual(testCase, ...
            compactData.optimizationInput.dijRef.numOfScenarios, ...
            expectedRef.numOfScenarios,mode);
        verifyEqual(testCase, ...
            compactData.optimizationInput.dijRef.numOfScenarios, ...
            cachedRef.cacheMetadata.numOfScenarios,mode);
    end
...
function testCompactDijRefsRejectPersistedPayloadContextMismatch(testCase)
modes = {'INTERVAL2','INTERVAL3','PROB2'};
for modeIx = 1:numel(modes)
    mode = modes{modeIx};
    [compactData,dataMetadata,runConfig,cachePath] = ...
        compactRobustnessFixture(testCase,mode);
    compactData.optimizationInput.dijRef.payloadContextHash = ...
        'changed-payload-context';
    ...
    verifyError(testCase,@() requireFullInput( ...
        ...
        runConfig,cachePath), ...
        ['planWorkflow:cache:DoseInfluenceCacheService:' ...
        'DijCacheRefClinicalMismatch'],mode);
end
end

function testCompactDijRefsRejectMissingFinalContractFields(testCase)
modes = {'INTERVAL2','INTERVAL3','PROB2'};
requiredFields = {'cstGeometryHash','stfHash','payloadContextHash'};
for modeIx = 1:numel(modes)
    mode = modes{modeIx};
    for fieldIx = 1:numel(requiredFields)
        ...
        [compactData,dataMetadata,runConfig,cachePath] = ...
            compactRobustnessFixture(testCase,mode);
        compactData.optimizationInput.dijRef = rmfield( ...
            compactData.optimizationInput.dijRef,fieldName);
        ...
        verifyError(testCase,@() requireFullInput( ...
            ...
            runConfig,cachePath), ...
            ['planWorkflow:cache:DoseInfluenceCacheService:' ...
            'DijCacheRefClinicalMismatch'], ...
            sprintf('%s:%s',mode,fieldName));
    end
end
end

function testCompactDijRefsRejectMissingPhysicalTag(testCase)
modes = {'INTERVAL2','INTERVAL3','PROB2'};
for modeIx = 1:numel(modes)
    mode = modes{modeIx};
    [compactData,dataMetadata,runConfig,cachePath] = ...
        compactRobustnessFixture(testCase,mode);
    compactData.optimizationInput.dijRef = rmfield( ...
        compactData.optimizationInput.dijRef,'cachePhysicalTag');
...
function testCompactDijRefsUsePersistedRefWhenPlanScenarioModelDrifts( ...
        testCase)
modes = {'INTERVAL2','INTERVAL3','PROB2'};
for modeIx = 1:numel(modes)
    mode = modes{modeIx};
    [compactData,dataMetadata,runConfig,cachePath] = ...
        compactRobustnessFixture(testCase,mode);
    compactData.pln.multScen = driftedWorkflowScenarioModel();
    compactData.optimizationInput.pln.multScen = ...
        driftedWorkflowScenarioModel();
...
function testCompactDijRefsRejectMissingScenarioContract( ...
        testCase)
modes = {'INTERVAL2','INTERVAL3','PROB2'};
for modeIx = 1:numel(modes)
    mode = modes{modeIx};
    ...
    missingFingerprint = compactData;
    missingFingerprint.optimizationInput.dijRef = rmfield( ...
        missingFingerprint.optimizationInput.dijRef,'scenarioFingerprint');
    verifyError(testCase,@() requireFullInput( ...
        planWorkflow.persistence.WorkflowDataArtifact.rehydrateAfterLoad( ...
        missingFingerprint,dataMetadata,runConfig,cachePath), ...
        runConfig,cachePath), ...
        ['planWorkflow:cache:DoseInfluenceCacheService:' ...
        'DijCacheRefClinicalMismatch'],mode);

    missingCount = compactData;
    missingCount.optimizationInput.dijRef = rmfield( ...
        missingCount.optimizationInput.dijRef,'numOfScenarios');
    verifyError(testCase,@() requireFullInput( ...
        planWorkflow.persistence.WorkflowDataArtifact.rehydrateAfterLoad( ...
        missingCount,dataMetadata,runConfig,cachePath), ...
        runConfig,cachePath), ...
        ['planWorkflow:cache:DoseInfluenceCacheService:' ...
        'DijCacheRefClinicalMismatch'],mode);
end
...
function testCompactPayloadSeparatesDiscoveryFromRehydrateScenario( ...
        testCase)
modes = {'INTERVAL2','INTERVAL3','PROB2'};
for modeIx = 1:numel(modes)
    mode = modes{modeIx};
    [data,~,cachePath,expectedRef] = robustnessPersistenceFixture( ...
        testCase,mode);
    cached = load(fullfile(cachePath,expectedRef.cacheRelativeFile));
    data.pln.multScen = driftedWorkflowScenarioModel();

    switch char(mode)
        case {'INTERVAL2','INTERVAL3'}
            [discoveryCompatible,discoveryReason] = ...
                planWorkflow.precompute.IntervalDoseInfluence.isCachePayloadCompatible( ...
                cached,data,'discovery',expectedRef);
            [rehydrateCompatible,rehydrateReason] = ...
                planWorkflow.precompute.IntervalDoseInfluence.isCachePayloadCompatible( ...
                cached,data,'rehydrate',expectedRef);
        case 'PROB2'
            [discoveryCompatible,discoveryReason] = ...
    ...
    verifyFalse(testCase,discoveryCompatible,mode);
    verifyNotEmpty(testCase,discoveryReason,mode);
    verifyTrue(testCase,rehydrateCompatible,mode);
    verifyEmpty(testCase,rehydrateReason,mode);
end
end

function testCompactDijRefsRejectPersistedScenarioMismatch(testCase)
modes = {'INTERVAL2','INTERVAL3','PROB2'};
for modeIx = 1:numel(modes)
    mode = modes{modeIx};
    [compactData,dataMetadata,runConfig,cachePath] = ...
    ...
    verifyError(testCase,@() requireFullInput( ...
        ...
        runConfig,cachePath), ...
        ['planWorkflow:cache:DoseInfluenceCacheService:' ...
        'DijCacheRefScenarioMismatch'],mode);
end
end
...
function testCompactDijRefsRehydrateNominalOptimizationInputFromDerivedCache( ...
        testCase)
modes = {'INTERVAL2','INTERVAL3','PROB2'};
for modeIx = 1:numel(modes)
    mode = modes{modeIx};
    ...
    switch char(mode)
        case {'INTERVAL2','INTERVAL3'}
            ...
            expectedVariable = 'dijIntervalContext';
        case 'PROB2'
            data.optimizationInput = ...
                planWorkflow.precompute.OptimizationInput.build( ...
                data.ct,data.cst,data.pln,data.stf, ...
            ...
            expectedKind = 'prob';
            expectedVariable = 'dijProbContext';
    end
    ...
    verifyEqual(testCase,compactData.optimizationInput.dijKind, ...
        'nominal',mode);
    verifyEqual(testCase, ...
        compactData.optimizationInput.dijRef.cacheKind, ...
        expectedKind,mode);
    verifyEqual(testCase, ...
        compactData.optimizationInput.dijRef.variableName, ...
        expectedVariable,mode);
    ...
    fullInput = requireFullInput( ...
        ...
        runConfig,cachePath);
    verifyEqual(testCase,fullInput.dij.totalNumOfBixels,3,mode);
    verifyEqual(testCase,fullInput.dijKind,'nominal',mode);
end
end
...
verifyTrue(testCase,isfield(cacheMetadata,'cacheClinicalContext'),mode);
verifyTrue(testCase,isfield(cacheMetadata.cacheClinicalContext, ...
    'cstGeometryHash'),mode);
verifyNotEmpty(testCase, ...
    cacheMetadata.cacheClinicalContext.cstGeometryHash,mode);
verifyTrue(testCase,isfield(cacheMetadata.cacheClinicalContext, ...
    'stfHash'),mode);
verifyNotEmpty(testCase,cacheMetadata.cacheClinicalContext.stfHash,mode);
...
function testCstGeometryIdentityIgnoresObjectivesAndTracksGeometry( ...
        testCase)
baseCst = multiScenarioCst(2);
baseHash = planWorkflow.cache.CacheIdentity.valueHash( ...
    planWorkflow.cache.CacheIdentity.cstGeometryIdentity(baseCst));

objectiveDrift = baseCst;
objectiveDrift{1,5}.Priority = 99;
objectiveDrift{1,6} = {struct('penalty',7)};
objectiveHash = planWorkflow.cache.CacheIdentity.valueHash( ...
    planWorkflow.cache.CacheIdentity.cstGeometryIdentity( ...
    objectiveDrift));
verifyEqual(testCase,objectiveHash,baseHash);

nameDrift = baseCst;
nameDrift{1,2} = 'PTV';
verifyNotEqual(testCase, ...
    planWorkflow.cache.CacheIdentity.valueHash( ...
    planWorkflow.cache.CacheIdentity.cstGeometryIdentity(nameDrift)), ...
    baseHash);
...
verifyNotEqual(testCase, ...
    planWorkflow.cache.CacheIdentity.valueHash( ...
    planWorkflow.cache.CacheIdentity.cstGeometryIdentity(roleDrift)), ...
    baseHash);

voxelDrift = baseCst;
voxelDrift{1,4}{1} = 'different-voxel-set';
verifyNotEqual(testCase, ...
    planWorkflow.cache.CacheIdentity.valueHash( ...
...
function testWorkflowDataArtifactDerivedRefsRejectOptimizationInputContextChange( ...
        testCase)
modes = {'INTERVAL2','INTERVAL3','PROB2'};
for modeIx = 1:numel(modes)
    mode = modes{modeIx};
    [compactData,dataMetadata,runConfig,cachePath] = ...
        compactRobustnessFixture(testCase,mode);
    compactData.optimizationInput.cst = {2};
...
function testWorkflowDataArtifactDerivedRefsAllowOptimizationInputObjectiveDrift( ...
        testCase)
modes = {'INTERVAL2','INTERVAL3','PROB2'};
for modeIx = 1:numel(modes)
    mode = modes{modeIx};
    [compactData,dataMetadata,runConfig,cachePath] = ...
        compactRobustnessFixture(testCase,mode);
    compactData.optimizationInput.cst{1,5}.Priority = 99;
    compactData.optimizationInput.cst{1,6} = ...
        {struct('className','DoseObjectives.matRad_SquaredDeviation', ...
        'penalty',7)};
...
function testCompactDijDiscoveryTreatsMissingRehydrationContextAsStale( ...
        testCase)
modes = {'INTERVAL2','INTERVAL3','PROB2'};
mutations = { ...
    struct('name','cacheClinicalContext', ...
    'reason','compact rehydration context'), ...
    struct('name','cstGeometryHash', ...
    'reason','compact CST geometry identity'), ...
    struct('name','scenarioFingerprint', ...
    'reason','compact scenario fingerprint'), ...
    struct('name','numOfScenarios', ...
    'reason','compact scenario count')};
for modeIx = 1:numel(modes)
    mode = modes{modeIx};
    for mutationIx = 1:numel(mutations)
        mutation = mutations{mutationIx};
        [data,runConfig,cachePath,expectedRef] = ...
            robustnessPersistenceFixture(testCase,mode);
        cacheFile = fullfile(cachePath,expectedRef.cacheRelativeFile);
        cached = load(cacheFile);
        ...
        switch mutation.name
            case 'cacheClinicalContext'
                cacheMetadata = rmfield(cacheMetadata,'cacheClinicalContext');
            case 'cstGeometryHash'
                cacheMetadata.cacheClinicalContext = rmfield( ...
                    cacheMetadata.cacheClinicalContext,'cstGeometryHash');
            otherwise
                cacheMetadata = rmfield(cacheMetadata,mutation.name);
        end
        saveCompactCachePayload(cacheFile,cached,mode,cacheMetadata);
        ...
        verifyFalse(testCase,cacheHit, ...
            sprintf('%s:%s',mode,mutation.name));
        verifyFalse(testCase,compatible, ...
            sprintf('%s:%s',mode,mutation.name));
        verifyNotEmpty(testCase, ...
            strfind(reason,mutation.reason), ... %#ok<STREMP>
            sprintf('%s:%s',mode,mutation.name));
    end
end
end

function testCompactDoseInfluenceCacheRefRequiresFinalMetadataContract( ...
        testCase)
[~,~,cachePath,expectedRef] = robustnessPersistenceFixture( ...
    testCase,'PROB2');
cacheFile = fullfile(cachePath,expectedRef.cacheRelativeFile);
...
verifyError(testCase,@() planWorkflow.cache.DoseInfluenceCacheRef.create( ...
    expectedRef.cacheKind,expectedRef.tag,cacheFile,cachePath, ...
    cacheMetadata,expectedRef.variables,expectedRef.totalNumOfBixels), ...
    ['planWorkflow:cache:DoseInfluenceCacheRef:' ...
    'IncompleteCompactRef']);
end

function testWorkflowDataArtifactCompactCacheRejectsMissingRequestedVariable( ...
        testCase)
[compactData,dataMetadata,runConfig,cachePath] = ...
    compactRobustnessFixture(testCase,'PROB2');
cacheFile = fullfile(cachePath, ...
    compactData.optimizationInput.dijRef.cacheRelativeFile);
cached = load(cacheFile,'cacheMetadata','dij_prob');
cacheMetadata = cached.cacheMetadata; %#ok<NASGU>
dij_prob = cached.dij_prob; %#ok<NASGU>
builtin('save',cacheFile,'dij_prob','cacheMetadata','-v7.3');
...
function scenarioModel = driftedWorkflowScenarioModel()
scenarioModel = workflowScenarioModel();
scenarioModel.shiftSD = [9 8 7];
scenarioModel.wcSigma = 2;
end
...
function saveCompactCachePayload(cacheFile,cached,mode,cacheMetadata)
switch char(mode)
    case {'INTERVAL2','INTERVAL3'}
        dij_interval = cached.dij_interval; %#ok<NASGU>
        dijIntervalContext = cached.dijIntervalContext; %#ok<NASGU>
