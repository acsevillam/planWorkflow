...
function testOptimizationResultArtifactRoundTripsCompletedUnit(testCase)
fixture = testCase.applyFixture( ...
    matlab.unittest.fixtures.TemporaryFolderFixture);
cachePath = fullfile(fixture.Folder,'cache');
runConfig = struct( ...
    'cacheRootPath',cachePath, ...
    'runId','optimization-artifact', ...
    'caseID','caseA', ...
    'workflowType','test');
optimizationInput = optimizationInputFixture();
unitInfo = struct( ...
    'role','robust', ...
    'planId','planA', ...
    'variantId','variantA', ...
    'label','Plan A Variant A', ...
    'planIndex',1, ...
    'variantIndex',2, ...
    'unitKey','robust_001_planA_002_variantA');
resultGUI = struct();
resultGUI.w = [1; 2; 3];
resultGUI.physicalDose = ones(2,2,1);

compactResult = ...
    planWorkflow.persistence.OptimizationResultArtifact.persistResult( ...
    resultGUI,runConfig,cachePath,unitInfo.unitKey,unitInfo, ...
    optimizationInput);

verifyEqual(testCase,compactResult.w,[1; 2; 3]);
verifyFalse(testCase,isfield(compactResult,'physicalDose'));
cacheFile = fullfile(cachePath,'optimization_results', ...
    [unitInfo.unitKey '.mat']);
verifyTrue(testCase,exist(cacheFile,'file') == 2);

[cached,found] = ...
    planWorkflow.persistence.OptimizationResultArtifact.cachedResult( ...
    runConfig,cachePath,unitInfo.unitKey,unitInfo,optimizationInput);

verifyTrue(testCase,found);
verifyEqual(testCase,cached.w,[1; 2; 3]);
verifyFalse(testCase,isfield(cached,'physicalDose'));

tmpFile = [cacheFile '.tmp'];
payload = struct('resultGUI',resultGUI); %#ok<NASGU>
builtin('save',tmpFile,'-struct','payload');
delete(cacheFile);
[~,foundTmpOnly] = ...
    planWorkflow.persistence.OptimizationResultArtifact.cachedResult( ...
    runConfig,cachePath,unitInfo.unitKey,unitInfo,optimizationInput);
verifyFalse(testCase,foundTmpOnly);
end
...
function input = optimizationInputFixture()
...
pln = struct( ...
    ...
    'machine','Generic', ...
    'propStf',struct('numOfBeams',1), ...
    'propOpt',struct('quantityOpt','physicalDose'));
dij = struct('totalNumOfBixels',3,'physicalDose',{{sparse(1,3)}});
