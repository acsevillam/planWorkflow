...
function testCancelledEditorAbortsGuiStage(testCase)
workflow = planWorkflowTest.EngineProbe(baseWorkflowConfig(testCase));
...
verifyError(testCase,@() workflow.gui(), ...
    'planWorkflow:gui:PlanEditor:Cancelled');
...
verifyEqual(testCase, ...
    planWorkflow.config.WorkflowParameterSchema.precomputeRobustParameterFields( ...
    'INTERVAL2','nomScen'), ...
    {'label','robustness','theta1','scen_mode','ctActive', ...
    'ctScenProb'});
verifyEqual(testCase, ...
    planWorkflow.config.WorkflowParameterSchema.precomputeRobustParameterFields( ...
    'INTERVAL3','nomScen'), ...
    {'label','robustness','theta1','theta2','KMode', ...
    'kmax','retentionThreshold','scen_mode','ctActive', ...
    'ctScenProb'});
verifyEqual(testCase, ...
    planWorkflow.config.WorkflowParameterSchema.precomputeRobustParameterFields( ...
    'INTERVAL3','nomScen',struct(),'static'), ...
    {'label','robustness','theta1','theta2','KMode', ...
    'kmax','scen_mode','ctActive','ctScenProb'});
end
...
verifyEqual(testCase, ...
    ...
    'reference_theta2','reference_KMode','reference_kmax', ...
    'reference_retentionThreshold','reference_scen_mode', ...
...
sectionFields = {'patientSection','imagesSection', ...
    ...
    'robustnessParameterSection','scenarioModeSection', ...
    'scenarioSelectionSection','scenarioParameterSection', ...
    'cacheSection'};
...
verifyEqual(testCase,template.objectiveSets.robustPlans(1).id,'robust_1');
...
verifyTrue(testCase,contains( ...
    ...
    'Scenario-space SVD truncation mode'));
verifyTrue(testCase,contains( ...
    planWorkflow.gui.HelpText.parameter('kmax'), ...
    'scenario-bixel working matrix'));
verifyTrue(testCase,contains( ...
...
verifyTrue(testCase,contains(interval3Help,'scenario Gram'));
