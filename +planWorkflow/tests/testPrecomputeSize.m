...
function testPrecomputeArtifactUsesTotalPrecomputingBytes(testCase)
referenceSize = planWorkflow.performance.PrecomputeSize.single( ...
    100,'reference','Reference','dij',[]);
inputSize = planWorkflow.performance.PrecomputeSize.single( ...
    200,'input','PROB2','dij_robust',referenceSize);
precomputeDij = struct();
precomputeDij.precomputeSize = struct( ...
    'compactBytes',40, ...
    'auxiliaryPeakBytes',60, ...
    'totalPrecomputingBytes',100);

probSize = planWorkflow.performance.PrecomputeSize.combine( ...
    inputSize,'derived','dij_prob',precomputeDij,'PROB2');

verifyEqual(testCase,probSize.totalSizeBytes,300,'AbsTol',1e-12);
verifyEqual(testCase,probSize.relativeSize,3,'AbsTol',1e-12);
verifyEqual(testCase,probSize.components(2).sizeBytes,100, ...
    'AbsTol',1e-12);
end
...
compactSize = planWorkflow.performance.PrecomputeSize.combine( ...
    inputSize,'derived','dij_prob',100,'Reference');
...
artifact = struct('precomputeSize',struct( ...
...
probSize = planWorkflow.performance.PrecomputeSize.combine( ...
    ...
    200,'input','PROB2','dij_robust',referenceSize), ...
    'derived','dij_prob',100,'PROB2');
cacheMetadata = struct('dijPrecomputingSize',probSize);
