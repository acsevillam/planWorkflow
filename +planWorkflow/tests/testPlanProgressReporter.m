...
precomputeDijSizeLabels = findall(robustSummaryPanels(1), ...
    'Style','text','String','Precompute dij size (MB)');
verifyNotEmpty(testCase,precomputeDijSizeLabels);
precomputeDijSizeValues = findall(robustSummaryPanels(1), ...
    'Style','edit','String','4.00');
verifyNotEmpty(testCase,precomputeDijSizeValues);
precomputeRelativeDijSizeLabels = findall(robustSummaryPanels(1), ...
    'Style','text','String','Precompute relative dij size');
verifyNotEmpty(testCase,precomputeRelativeDijSizeLabels);
precomputeRelativeDijSizeValues = findall(robustSummaryPanels(1), ...
    'Style','edit','String','2.000');
verifyNotEmpty(testCase,precomputeRelativeDijSizeValues);
...
precomputeDetailValues = findEditControlsContaining( ...
    robustSummaryPanels(1), ...
    {'dij_robust','dijPrecomputingTiming','dijPrecomputingSize'});
...
referenceDijSize = planWorkflow.performance.PrecomputeSize.single( ...
    2 * 1024^2,'reference','Reference','dij',[]);
precomputeDijSize = planWorkflow.performance.PrecomputeSize.single( ...
    4 * 1024^2,'robust','INTERVAL2','dij_robust',referenceDijSize);
precompute.detail = ...
    jsonencode(struct( ...
    'dij_robust',struct('numberOfScenarios',1, ...
    'matrix',struct('dimensions','1x3')), ...
    'dijPrecomputingTiming',precomputeDijTiming, ...
    'dijPrecomputingSize',precomputeDijSize));
