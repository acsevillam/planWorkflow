classdef DosePullingReportLogger
    ...
    methods (Static)
        function logReference(logFn,report)
            ...
            if isfield(report,'selected') && ~isempty(report.selected)
                finalEntry = report.selected;
            end
        ...
        function logRobust(logFn,report)
            ...
            for planIx = 1:numel(report.plans)
                ...
                if isfield(planReport,'selected') && ...
                        ~isempty(planReport.selected)
                    finalMetrics = planReport.selected;
                end
                logFn(sprintf(['Dose pulling step 2 plan %d results: ' ...
