classdef SamplingService
    ...
    methods (Static)
        function [ctSampling,cstSampling,normalizationReport] = ...
            ...
            if runConfig.sampling_linkToOptimization
                ...
                if planWorkflow.sampling.SamplingService.useLinkedGeometry( ...
                        runConfig,optimizationInput)
                    ...
                    planWorkflow.sampling.SamplingService.logSingleCtSampling( ...
                        context,ctSampling);
                    return;
                end

                planWorkflow.sampling.SamplingService.log( ...
                    context,['Sampling CT scenarios are active, but ' ...
                    'linked optimization geometry is a CT reference view; ' ...
                    'loading full sampling geometry.']);
            end

            [ctSampling,cstSampling,normalizationReport] = ...
                planWorkflow.sampling.SamplingService.loadFullSamplingGeometry( ...
                context);
            planWorkflow.sampling.SamplingService.logSingleCtSampling( ...
                context,ctSampling);
        end
        ...
        function [finishedScenarios,totalScenarios] = latestProgress(messages)
            ...
            for i = 1:numel(messages)
                tokens = ...
                    planWorkflow.sampling.SamplingService.progressTokens( ...
                    messages{i});
                if ~isempty(tokens)
        ...
        function tokens = progressTokens(message)
            tokens = regexp(message, ...
                ['matRad:\s*Sampling progress:\s*(\d+)\s*/\s*' ...
                 '(\d+)\s*scenarios?\.'], ...
                'tokens','once');
        end
    end
    ...
    methods (Static, Access = private)
        ...
        function tf = useLinkedGeometry(runConfig,optimizationInput)
            tf = true;
            if ~planWorkflow.sampling.SamplingService.samplingCtActive( ...
                    runConfig)
                return;
            end

            if planWorkflow.sampling.SamplingService.isCtReferenceView( ...
                    optimizationInput)
                tf = false;
            end
        end

        function tf = samplingCtActive(runConfig)
            tf = true;
            if isstruct(runConfig) && isfield(runConfig,'sampling_ctActive') && ...
                    ~isempty(runConfig.sampling_ctActive)
                tf = logical(runConfig.sampling_ctActive);
            end
        end

        function tf = isCtReferenceView(optimizationInput)
            tf = isstruct(optimizationInput) && ...
                isfield(optimizationInput,'ctReferenceView') && ...
                isstruct(optimizationInput.ctReferenceView) && ...
                isfield(optimizationInput.ctReferenceView,'active') && ...
                logical(optimizationInput.ctReferenceView.active);
        end

        function logSingleCtSampling(context,ctSampling)
            if ~planWorkflow.sampling.SamplingService.samplingCtActive( ...
                    context.runConfig)
                return;
            end
            ...
            numOfCtScen = ...
                planWorkflow.sampling.SamplingService.numOfCtScen( ...
                ctSampling);
            if isempty(numOfCtScen) || numOfCtScen > 1
                return;
            end

            planWorkflow.sampling.SamplingService.log( ...
                context,['Sampling CT scenarios are active, but the ' ...
                'effective sampling geometry contains only one CT ' ...
                'scenario.']);
        end

        function numOfCtScen = numOfCtScen(ct)
            numOfCtScen = [];
            if isstruct(ct) && isfield(ct,'numOfCtScen') && ...
        ...
        function log(context,message)
            if isstruct(context) && isfield(context,'log') && ...
                    isa(context.log,'function_handle')
                context.log(char(message));
            end
        end
    end
end
