classdef ResourceDetails
    ...
    methods (Static)
        function detail = planTask(stageName,taskName,taskOutputs)
            ...
            elseif strcmp(char(stageName),'pullDose') && ...
                    strcmp(char(taskName),'dosePulling')
                detailData = ...
                    planWorkflow.performance.ResourceDetails.dosePullingTask( ...
                    taskOutputs);
            elseif strcmp(char(stageName),'optimize') && ...
                ...
                detailData = ...
                    ...
                    taskOutputs);
            elseif strcmp(char(stageName),'sample') && ...
                    strcmp(char(taskName),'sampling')
                detailData = ...
                    planWorkflow.performance.ResourceDetails.samplingTask( ...
                    taskOutputs);
            end
            ...
            switch char(taskName)
                case 'doseInfluence'
                    detailData = ...
                        ...
                        taskOutputs,1));
                    detailData = ...
                        planWorkflow.performance.ResourceDetails.appendDijPrecomputingTiming( ...
                        detailData, ...
                        planWorkflow.performance.ResourceDetails.taskOutput( ...
                        taskOutputs,2));
                    detailData = ...
                        planWorkflow.performance.ResourceDetails.appendDijPrecomputingSize( ...
                        detailData, ...
                        planWorkflow.performance.ResourceDetails.taskOutput( ...
                        taskOutputs,3));
                    detailData = ...
                        planWorkflow.performance.ResourceDetails.appendCacheTelemetry( ...
                        detailData, ...
                        planWorkflow.performance.ResourceDetails.taskOutput( ...
                        taskOutputs,4));
                case 'robustDoseInfluence'
                    ...
                    detailData = ...
                        ...
                        planWorkflow.performance.ResourceDetails.taskOutput( ...
                        taskOutputs,4));
                case 'nominalDoseInfluence'
                    detailData = ...
                        planWorkflow.performance.ResourceDetails.appendDoseInfluence( ...
                        detailData,'dij_nominal', ...
                        planWorkflow.performance.ResourceDetails.taskOutput( ...
                        taskOutputs,1));
                    detailData = ...
                        planWorkflow.performance.ResourceDetails.appendDijPrecomputingTiming( ...
        ...
        function detailData = dosePullingTask(taskOutputs)
            detailData = struct();
            report = planWorkflow.performance.ResourceDetails.taskOutput( ...
                taskOutputs,2);
            summary = ...
                planWorkflow.performance.ResourceDetails.dosePullingReport( ...
                report);
            if ~isempty(fieldnames(summary))
                detailData.dosePulling = summary;
            end
        end
        ...
        function detailData = samplingTask(taskOutputs)
            detailData = struct();
            sample = planWorkflow.performance.ResourceDetails.taskOutput( ...
                taskOutputs,1);
            if isstruct(sample) && isfield(sample,'pln') && ...
                    isstruct(sample.pln) && ...
                    isfield(sample.pln,'samplingMemoryEstimate')
                detailData.samplingMemoryEstimate = ...
                    sample.pln.samplingMemoryEstimate;
            end
        end

        function summary = dosePullingReport(report)
            summary = struct();
            if ~isstruct(report)
                return;
            end

            fields = {'converged','iterations','stopReason','trace'};
            for fieldIx = 1:numel(fields)
                fieldName = fields{fieldIx};
                if isfield(report,fieldName)
                    summary.(fieldName) = report.(fieldName);
                end
            end

            if isfield(report,'plans') && iscell(report.plans)
                plans = cell(1,numel(report.plans));
        ...
        function detailData = appendRobustData(detailData,robustData, ...
            ...
            if isfield(robustData,'dijPrecomputingTiming')
                detailData = ...
                    planWorkflow.performance.ResourceDetails.appendDijPrecomputingTiming( ...
                    detailData,robustData.dijPrecomputingTiming);
            end
            if isfield(robustData,'dijPrecomputingSize')
                detailData = ...
                    planWorkflow.performance.ResourceDetails.appendDijPrecomputingSize( ...
                    detailData,robustData.dijPrecomputingSize);
            end
            detailData = ...
                planWorkflow.performance.ResourceDetails.appendCacheTelemetry( ...
                detailData,robustData);
        end

        function detailData = appendDijPrecomputingTiming( ...
                detailData,timing)
            [timing,tf] = ...
                planWorkflow.performance.PrecomputeTiming.normalize(timing);
            if tf
                detailData.dijPrecomputingTiming = timing;
            end
        end

        function detailData = appendDijPrecomputingSize( ...
                detailData,sizeData)
            [sizeData,tf] = ...
                planWorkflow.performance.PrecomputeSize.normalize(sizeData);
            if tf
                detailData.dijPrecomputingSize = sizeData;
            end
        end
        ...
        function detailData = appendCacheTelemetry(detailData,value)
            telemetry = [];
            if isstruct(value) && isfield(value,'cacheTelemetry')
                telemetry = value.cacheTelemetry;
            elseif isstruct(value) && isfield(value,'dijCacheRefs') && ...
                    isstruct(value.dijCacheRefs)
                refs = value.dijCacheRefs;
                refFields = fieldnames(refs);
                telemetry = struct();
                telemetry(:) = [];
                for refIx = 1:numel(refFields)
                    ref = refs.(refFields{refIx});
                    if isstruct(ref) && isfield(ref,'cacheTelemetry')
                        telemetry(end + 1) = ref.cacheTelemetry; %#ok<AGROW>
                    end
                end
            end
            ...
            detailData.cacheTelemetry = telemetry;
        end
