classdef PlanProgressReporter < handle
        ...
        function performance = resultPerformance(results)
            ...
            if isstruct(results) && isfield(results,'performance')
                ...
                if isstruct(performance) && ...
                    ...
                    planTimings = ...
                        planWorkflow.performance.PrecomputeSize.enrich( ...
                        planTimings);
                    performance.planTimings = ...
    ...
    methods (Access = private)
        ...
        function timing = dijPrecomputeTiming(obj,timings)
            ...
            for i = 1:numel(timings)
                ...
                if obj.isFiniteTimingValue( ...
                        ...
                        obj.isFiniteTimingValue( ...
                        candidate,'dijPrecomputingSizeBytes')
                    timing = candidate;
                end
            end
        end
        ...
        function rows = appendDijPrecomputeTimingRows(obj,rows,timing)
            ...
            if obj.isFiniteTimingValue(timing,'dijPrecomputingSizeBytes')
                rows(end + 1,:) = {'Precompute dij size (MB)', ...
                    planWorkflow.gui.ValueFormat.megabytes( ...
                    timing.dijPrecomputingSizeBytes),'', ...
                    ['Estimated MATLAB payload size attributed to dose ' ...
                     'influence precompute for this plan.']};
            end
            if obj.isFiniteTimingValue(timing,'relativeDijPrecomputingSize')
                rows(end + 1,:) = {'Precompute relative dij size', ...
                    planWorkflow.gui.ValueFormat.ratio( ...
                    timing.relativeDijPrecomputingSize),'', ...
                    ['Dose influence precompute size normalized to the ' ...
                     'reference plan dij precompute size.']};
            end
        end
