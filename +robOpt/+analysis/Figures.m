    ...
    methods (Static)
        function figureFiles = saveSamplingAnalysisFigures(analysisFolder,label, ...
            ...
            figureFiles = struct( ...
                ...
                'dvhMultiscenario','', ...
                'dvhTrustband','');
            figureFiles.gamma = robOpt.analysis.Figures.saveFigureIfValid( ...
                gammaFig,fullfile(analysisFolder,[label '_gamma.fig']));
            figureFiles.robustness1 = robOpt.analysis.Figures.saveFigureIfValid( ...
                robustnessFig1,fullfile(analysisFolder,[label '_robustness1.fig']));
            figureFiles.robustness2 = robOpt.analysis.Figures.saveFigureIfValid( ...
                robustnessFig2,fullfile(analysisFolder,[label '_robustness2.fig']));
            figureFiles.meanDose = robOpt.analysis.Figures.saveDoseStatisticFigure( ...
                samplingData.ct,samplingData.cst,doseStat.meanCubeW, ...
                analysis.doseWindow,slice, ...
                fullfile(analysisFolder,[label '_mean_dose.fig']), ...
                ['Mean dose for ' label], ...
                'Expected Dose [Gy]',1.5);
            figureFiles.stdDose = robOpt.analysis.Figures.saveDoseStatisticFigure( ...
                samplingData.ct,samplingData.cst,doseStat.stdCubeW, ...
            ...
            dvhScale = robOpt.analysis.Figures.samplingDvhPlotScale( ...
                sample,analysis);
            figureFiles.dvhMultiscenario = ...
                robOpt.analysis.Figures.saveSamplingDvhFigure( ...
                samplingData.cst,sample,dvhScale,analysis.doseWindowDvh, ...
                'multiscenario', ...
        ...
        function filePath = saveDoseStatisticFigure(ct,cst,doseCube,doseWindow, ...
            ...
            doseWindow = robOpt.analysis.Figures.resolveDoseWindow( ...
                doseCube,doseWindow);
            doseIsoLevels = robOpt.analysis.Figures.resolveDoseIsoLevels( ...
                doseCube);
            ...
            if numSlices > 1
                sliderStep = robOpt.analysis.Figures.sliderStep(numSlices);
            ...
            filePath = robOpt.analysis.Figures.saveFigureIfValid(fig,filePath);
        end

        function filePath = saveSamplingDvhFigure(cst,sample,dvhScale, ...
            ...
            matRad_showDVHFromSampling(sample.caSamp,dvhScale,cst,sample.pln, ...
        ...
        function dvhScale = samplingDvhPlotScale(sample,analysis)
            if isfield(sample,'resultGUINomScen') && ...
                    isfield(sample.resultGUINomScen,'analysisDoseScale')
                dvhScale = 1;
            else
                dvhScale = matRad_getAnalysisDoseScale( ...
                    sample.pln,analysis.doseMode,[]);
            end
        end
